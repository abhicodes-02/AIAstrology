...
export default function KpVarshaphalDashboardView({ 
  ...
  const handleDownloadPDF = async () => {
    ...
    try {
      ...
      link.href = url;
      link.download = `KP_Varshaphal_${name.replace(/\s+/g, '_')}.pdf`;
      document.body.appendChild(link);
      link.click();
      document.body.removeChild(link);
      URL.revokeObjectURL(url);
    } catch (error) {
      console.error("Error generating KP Varshaphal PDF", error);
    } finally {
  ...
  return (
    <div className="min-h-screen bg-transparent text-indigo-100 font-sans relative overflow-x-hidden selection:bg-yellow-500/30">
      ...
      <div className="relative z-10 max-w-[1400px] mx-auto px-4 sm:px-6 lg:px-8 py-8 md:py-12" ref={printRef}>
        {/* Navigation & Header (Identical structure to Vedic Varshaphal) */}
        <motion.div 
        ...
        >
          <div>
            <Link href={`/kp-kundli?${queryParams}`} className="group inline-flex items-center gap-2 text-yellow-400 hover:text-yellow-300 transition-all text-sm font-medium mb-6 bg-yellow-500/10 px-4 py-2 rounded-full border border-yellow-500/20 hover:border-yellow-500/40">
            ...
            <div className="flex items-center gap-3">
              ...
              <span className="hidden sm:inline-block px-3 py-1 text-xs font-bold rounded-full bg-yellow-500/20 text-yellow-300 border border-yellow-500/30 tracking-wider">
                SUB-LORD CYCLE
              </span>
            </div>
            <p className="text-yellow-200/60 mt-2 text-lg">
              {name} • KP Varshaphal Annual Forecast (Age {data.age || "Annual Cycle"})
            </p>
          </div>
          ...
          <div className="flex flex-wrap items-center gap-3">
            <Link href={`/varshaphal?${queryParams}`}>
              ...
              >
                <Sparkles className="w-4 h-4 mr-2 text-indigo-400" /> Switch to Vedic Varshaphal
              </Button>
            </Link>
            <Link href={`/kp-daily-insight?${queryParams}`}>
              ...
              >
                <Compass className="w-4 h-4 mr-2 text-cyan-400" /> KP Daily Insight
              </Button>
            </Link>
            <Button 
              variant="outline" 
        ...
        >
              ...
              <div className="flex items-center gap-4 mb-8">
                ...
                <div>
                  <h3 className="text-3xl md:text-4xl font-space font-bold text-transparent bg-clip-text bg-gradient-to-r from-yellow-200 to-orange-400">
                    The Year Ahead (KP Sub-Lord Arc)
                  </h3>
                  <p className="text-yellow-200/70 text-lg mt-1">
                    Solar Return Sign: <strong className="text-yellow-100">{data.solarReturnSign}</strong> | Sub-Lord: <strong className="text-amber-300">{data.solarReturnSubLord}</strong>
                  </p>
                </div>
              </div>
          ...
          {/* Monthly Breakdown Grid (Full 12 Months matching Vedic Layout) */}
          <motion.div variants={itemVariants} className="space-y-8">
            <div className="flex items-center gap-4 px-2">
              <CalendarDays className="w-8 h-8 text-indigo-400" />
              <div>
                <h3 className="text-3xl font-space font-bold text-indigo-100">12-Month KP Progression</h3>
                <p className="text-indigo-300/60">How your Varshaphal manifests dynamically under active Sub-Lords</p>
              </div>
            </div>
              ...
              {monthlyData.map((monthData: any, index: number) => (
                ...
                >
                  ...
                  <div className="relative z-10 flex flex-col h-full space-y-6">
                    ...
                    {/* Specifics: Career & Relationships */}
